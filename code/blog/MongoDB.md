...
- **C**reate → Crear documentos.
- **R**ead → Leer datos.
- **U**pdate → Actualizar documentos.
- **D**elete → Eliminar documentos.
...
- **Documento**: Es la unidad básica de almacenamiento en MongoDB. Está representado en formato **BSON** (similar a JSON).
- **Colección**: Conjunto de documentos relacionados, similar a una tabla en bases de datos relacionales.
...
- **BSON (Binary JSON)**: Es el formato binario que usa MongoDB internamente.
  - Soporta **tipos de datos adicionales** como `ObjectId`, `Date`, `Decimal128`, `Binary`.
  - Más eficiente en almacenamiento y velocidad.
- **JSON**: Solo texto plano, más limitado en tipos de datos.
...
- Tiene **12 bytes**, distribuidos así:
  1. **4 bytes** → Marca de tiempo (timestamp).
  2. **5 bytes** → Identificador de máquina y proceso.
  3. **3 bytes** → Contador incremental.
...
1. Crear un índice en el campo `fecha`:
   ```js
...
- **Amigos**: Array embebido, porque suele consultarse junto al perfil.
- **Likes**: Guardar solo los IDs para evitar duplicidad y mantener la consulta rápida.
