...
- **Sistemas de gestión de usuarios**: registro, login y control de roles.
- **E-commerce**: catálogos de productos, carritos de compra y órdenes.
- **Sistemas de reservas**: hoteles, vuelos, turnos médicos, etc.
- **Gestión de contenidos**: blogs, portales de noticias o sitios corporativos.
- **Aplicaciones SaaS**: paneles de administración y sistemas de seguimiento.
...
- **MongoDB** → cuando los datos son muy **flexibles y no estructurados**.
- **PostgreSQL** → para **consultas complejas** y datos muy relacionados.
- **SQLite** → en **aplicaciones locales o prototipos**.
...
**MySQL** es una solución potente y confiable para la gestión de bases de datos relacionales, ideal para una amplia gama de proyectos.
...
- **DELETE:** Elimina filas específicas de una tabla usando una condición `WHERE`. Mantiene la estructura y permite deshacer la operación si está dentro de una transacción.  
...
- **TRUNCATE:** Elimina _todos los registros_ de una tabla de forma rápida y resetea el contador de auto incremento. No se puede usar con `WHERE`.
...
- **DROP:** Elimina por completo una tabla, incluyendo su estructura y datos.
...
- **PRIMARY KEY:** Identifica de manera única cada registro en una tabla. No permite valores duplicados ni nulos.
...
- **CHAR(n):** Guarda cadenas con longitud fija. Completa con espacios si el valor es más corto.  
  _Uso:_ Ideal para campos con longitud constante (códigos, abreviaturas).

- **VARCHAR(n):** Guarda cadenas con longitud variable. Solo utiliza el espacio necesario.  
...
- **WHERE:** Filtra filas antes de agrupar los datos (`GROUP BY`).
- **HAVING:** Filtra resultados _después_ de aplicar funciones de agregación.
...
- **RIGHT JOIN:** Igual que LEFT pero en sentido opuesto.
- **FULL JOIN:** Devuelve todos los registros coincidentes o no (no soportado nativamente en MySQL).
...
- **Definición:** Estructuras que optimizan la búsqueda de datos.
- **Beneficio:** Aceleran las consultas `SELECT` y búsquedas con `WHERE`.
- **Desventaja:** Ocupan espacio y pueden ralentizar `INSERT`, `UPDATE` y `DELETE`.
...
- **Definición:** Bloque de SQL almacenado en la base para reutilización.
- **Uso:** Automatizar procesos repetitivos o complejos.
...
- **1FN:** Cada campo debe contener solo valores atómicos.
- **2FN:** Cada campo debe depender completamente de la clave primaria.
...
- **Subconsulta normal:** Se ejecuta una sola vez antes de la consulta principal.
- **Subconsulta correlacionada:** Se ejecuta para cada fila procesada por la consulta externa.
...
- **Beneficios:** Consultas más rápidas y administración más sencilla.
- **Desventajas:** Mayor complejidad en el diseño.
